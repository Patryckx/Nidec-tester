...
def send_command(serial_port, command_id, message_data=None):
    ...
    # Leer la respuesta
    response = serial_port.read(32)  # Leer hasta 32 bytes (ajustar si necesario)
    print(f"Respuesta: {response.hex().upper()}")
    return response

'''def parse_firmware_response(response):
    """Analiza la respuesta del comando de consulta de versión de firmware."""
    if response[3] == 0x76:  # ID de respuesta del firmware
        firmware_version = response[5:9]  # Ajustar según formato del firmware
        print(f"Versión de firmware: {firmware_version.hex().upper()}")
    else:
        print("Respuesta inesperada o inválida.")'''

def parse_firmware_response(response):
    """Analiza la respuesta del comando de consulta de versión de firmware."""
    if len(response) < 4:
        print(f"Respuesta demasiado corta: {response.hex().upper()}")
        return
    
    if response[3] == 0x76:  # ID de respuesta del firmware (0x76)
        # Asegurarse de que la respuesta tenga suficiente longitud para incluir datos de versión
        if len(response) >= 9:  # Ajustar según el formato esperado
            firmware_version = response[5:9]  # Extraer versión del firmware (ajustar si necesario)
            print(f"Versión de firmware: {firmware_version.hex().upper()}")
        else:
            print(f"Respuesta incompleta: {response.hex().upper()}")
    else:
        print(f"Respuesta inesperada: {response.hex().upper()}")
...
def main():
    ...
    try:
        with serial.Serial(port, baudrate, timeout=timeout) as ser:
            ...
            response = send_command(ser, 0x56)
